...
def getWordScore(word, n):
    ...
    if word == '':
        ...
        score = 0
        for item in word:
            score = score + SCRABBLE_LETTER_VALUES[item]
        score = score * length
        if length == n:
            score += 50
        return score
...
def isValidWord(word, hand, wordList):
    ...
    if word != '' and word in wordList and isWordInHand(word, hand) == True:
        return True
    else:
        return False
...
def playGame(wordList):
    ...
    init = raw_input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ').lower()
